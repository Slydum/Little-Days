import { applyChildhoodEffect, childhoodEventForState, commitChildhoodEvent, ensureChildhoodState } from "./childhood-v5.js?v=1";
    ...
    if (person) {
      ...
      if (Math.abs(effect.delta || 0) >= 2 || effect.note) {
        person.history.push({
          ...
          date: { ...state.date },
          eventId: "childhood-v4",
          key: effect.key,
          delta: effect.delta || 0,
          note: effect.note || null,
        });
        person.history = person.history.slice(-36);
      }
    }
  }
}
