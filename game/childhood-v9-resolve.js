import { applyChildhoodEffect, childhoodEventForState, commitChildhoodEvent, ensureChildhoodState } from "./childhood-v11.js?v=1";
    ...
    if (person) {
      ...
      if (Math.abs(effect.delta || 0) >= 2 || effect.note) {
        person.history.push({
          ...
          date: { ...state.date },
          eventId: "childhood-v9",
          key: effect.key,
          delta: effect.delta || 0,
          note: effect.note || null,
        });
        person.history = person.history.slice(-60);
      }
    }
  }
}
...
export function resolveChildhoodChoice(state, choiceId) {
  ...
  state.history.push({
    ...
    educationMode: state.childhood?.schoolV2?.mode || null,
  });
  state.history = state.history.slice(-700);
