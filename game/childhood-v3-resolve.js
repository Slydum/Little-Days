import { applyChildhoodEffect, childhoodEventForState, commitChildhoodEvent, ensureChildhoodState } from "./childhood-v4.js?v=1";
    ...
    if (person) {
      ...
      if (Math.abs(effect.delta || 0) >= 2 || effect.note) {
        person.history.push({
          ...
          date: { ...state.date },
          eventId: "childhood-v3",
          key: effect.key,
          delta: effect.delta || 0,
          note: effect.note || null,
        });
        person.history = person.history.slice(-30);
      }
    }
  }
}
...
export function resolveChildhoodChoice(state, choiceId) {
  ...
  state.history.push({
    ...
    continuity: event.relationshipContinuityKey ? "relationship-continuity-v2" : "childhood-v3",
    personId: event.relationshipPersonId || event.childhoodPersonId || null,
  });
  state.history = state.history.slice(-360);
