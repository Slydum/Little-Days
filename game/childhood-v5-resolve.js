import { applyChildhoodEffect, childhoodEventForState, commitChildhoodEvent, ensureChildhoodState } from "./childhood-v6.js?v=1";
import { syncPsychologyEventIntegration } from "./psychology-events.js?v=1";
    ...
    if (person) {
      ...
      if (Math.abs(effect.delta || 0) >= 2 || effect.note) {
        person.history.push({
          ...
          date: { ...state.date },
          eventId: "childhood-v5",
          key: effect.key,
          delta: effect.delta || 0,
          note: effect.note || null,
        });
        person.history = person.history.slice(-40);
      }
    }
  }
}
...
export function resolveChildhoodChoice(state, choiceId) {
  ...
  state.history = state.history.slice(-480);
